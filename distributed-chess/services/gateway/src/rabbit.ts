...
const PARTITIONS = 3; // Number of partitions for load balancing

const connectWithRetry = async (retries: number = 5, delay: number = 3000) => {
...
export const subscribeEvents = async (cb: (msg: any) => void) => {
  ...
  channel.consume(
    ...
    { noAck: true }
  ); // gateway subscription is ok with noAck (read-only fanout)
};
