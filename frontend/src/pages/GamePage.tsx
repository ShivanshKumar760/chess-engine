...
//         if (data.gameOver) {
//           setGameOver(true);
//           // GAME_OVER event will follow with winner details
//           // Set a fallback message in case it's delayed
//           if (!gameOverMessage) {
...
//         } else if (data.winner) {
//           setGameOverMessage(`${data.winner} wins!`);
//           toast(`${data.winner} wins! Better luck next time!`, { icon: "😔" });
//         }
//       }
//       if (data.type === "ERROR") {
...
//             gameOver ? "completed" : status === "active" ? "active" : "waiting"
//           }`}
//         >
//           {gameOver
...
//           <div
//             className={`player-avatar ${myColor === "w" ? "black" : "white"}`}
//           >
//             {myColor === "w"
...
//         <div className={`player-bar ${isMyTurn && !gameOver ? "active" : ""}`}>
//           <div
//             className={`player-avatar ${myColor === "w" ? "white" : "black"}`}
//           >
...
//               </div>
//               <div>
//                 <strong>Check:</strong> {game.isCheck() ? "⚠️ Yes" : "No"}
//               </div>
//               <div>
...
//               </div>
//               <div>
//                 <strong>Total Moves:</strong> {game.history().length}
//               </div>
//             </div>
//           </div>
//         </div>
...
export default function GamePage() {
  ...
  const gameRef = useRef(game);
  const pendingFenRef = useRef<string | null>(null);
  // Track if color has been set from HTTP so WS JOINED doesn't override it incorrectly
  ...
  // Load game data from HTTP — source of truth for initial state
  useEffect(() => {
    if (!token || !gameId) return;
    getGameDetails(token, gameId)
      .then((data) => {
        ...
        // Set color from HTTP — most reliable source
        const color = data.whitePlayer === user?.username ? "w" : "b";
        setMyColor(color);
        colorSetRef.current = true;

        // Restore board state from stored moves (handles refresh)
        if (data.moves && data.moves.length > 0) {
          try {
            ...
            setGame(restoredChess);
            buildMoveHistory(restoredChess);
          } catch (err) {
            console.error("Error restoring board from moves:", err);
          }
        }

        // If game is already completed, restore game over state
  ...
  }, [token, gameId, user, buildMoveHistory]);

  // WebSocket connection — only after HTTP data confirmed loaded
  useEffect(() => {
    if (!token || !gameId || loading || !gameData) return;

    // Don't connect WS if game is already over — no need for real-time
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === "JOINED") {
        // JOINED now carries the correct color from the server
        // Only use it if color wasn't already set from HTTP, OR confirm it matches
        if (data.color === "w" || data.color === "b") {
          setMyColor(data.color);
        }

        // Restore board from FEN sent by server (handles refresh mid-game)
        if (data.fen && data.fen !== "start") {
          try {
            const restoredGame = new Chess(data.fen);
            setGame(restoredGame);
            buildMoveHistory(restoredGame);
          } catch (err) {
            console.error("Error restoring FEN from JOINED:", err);
          }
        }

        // Restore game over state if game was already completed when reconnecting
        if (data.status === "completed") {
          setGameOver(true);
          if (data.winner === "draw") {
            setGameOverMessage("Game ended in a draw!");
        ...
        toast.success(
          `Playing as ${data.color === "w" ? "White ♔" : "Black ♚"}`
        );
      }

      if (data.type === "GAME_UPDATE") {
        // Server confirmed the move — clear pending and sync to server FEN
        pendingFenRef.current = null;
        const newGame = new Chess(data.fen);
        setGame(newGame);
        buildMoveHistory(newGame);
        if (data.gameOver) {
          setGameOver(true);
          // GAME_OVER event will follow shortly with winner details
          // Set fallback message in case it doesn't
          setGameOverMessage((prev) => prev || "Game Over");
        }
      }
  ...
  const onDrop = (sourceSquare: Square, targetSquare: Square): boolean => {
    ...
    try {
      ...
      // Apply optimistically and remember pre-move FEN for rollback
      pendingFenRef.current = preMovefen;
      setGame(gameCopy);
      buildMoveHistory(gameCopy);
  ...
  const whitePlayer = gameData?.whitePlayer || "Waiting...";
  const blackPlayer = gameData?.blackPlayer || "Waiting...";
  const isMyTurn = !gameOver && game.turn() === myColor;
  const status = gameData?.status || "waiting";
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            >
              <div>
                <strong>Turn:</strong>{" "}
                {gameOver ? "—" : game.turn() === "w" ? "White" : "Black"}
              </div>
              <div>
                <strong>Check:</strong> {game.isCheck() ? "⚠️ Yes" : "No"}
              </div>
